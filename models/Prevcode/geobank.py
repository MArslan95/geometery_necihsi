...
# class GeometryBank(nn.Module):
#     """
#     Spectral-Guided Low-Rank Geometry Bank
...
#     - eigvals:            (r,)
#     - res_var:            scalar
#     - spectral_proto:     (S,) optional
#     - band_importance:    (S,) optional

#     Critical design choice:
#     - this is an nn.Module with registered buffers
#     - therefore the geometry memory is part of model.state_dict()
#     - checkpoints now preserve the actual class memory
#     """

#     def __init__(self, d_model: int, rank: int, device: str = "cpu"):
#         super().__init__()
#         self.d_model = int(d_model)
#         self.rank = int(rank)
...
#     # Internal helpers
#     # ============================================================
#     def _empty_like_bank(self, shape, dtype=None):
#         dtype = self.means.dtype if dtype is None else dtype
...
#     def _prepare_mean(self, mean) -> torch.Tensor:
#         mean = torch.as_tensor(mean, device=self.device, dtype=self.means.dtype if self.means.numel() > 0 else torch.float32)
#         mean = mean.flatten()
#         if mean.numel() != self.d_model:
#             raise ValueError(f"mean must have {self.d_model} values, got {mean.numel()}")
#         return mean
...
#     def _sanitize_basis(self, basis: torch.Tensor) -> torch.Tensor:
#         """
#         Enforce shape (D, rank) and keep active columns orthonormal.
#         """
#         basis = torch.as_tensor(
#             basis,
#             device=self.device,
#             dtype=self.means.dtype if self.means.numel() > 0 else torch.float32,
#         )
...
#         # orthonormalize active columns only
#         col_norms = basis.norm(dim=0)
#         active = col_norms > 1e-8

#         out = torch.zeros((self.d_model, self.rank), device=self.device, dtype=basis.dtype)
#         if active.any():
#             active_basis = basis[:, active]
#             q, _ = torch.linalg.qr(active_basis, mode="reduced")
#             keep = min(q.size(1), self.rank)
#             out[:, :keep] = q[:, :keep]

#         return out

#     def _prepare_eigvals(self, eigvals) -> torch.Tensor:
...
#         return eigvals.clamp_min(1e-4)

#     def _prepare_res_var(self, res_var) -> torch.Tensor:
#         res_var = torch.as_tensor(
#             res_var,
#             device=self.device,
#             dtype=self.means.dtype if self.means.numel() > 0 else torch.float32,
#         ).reshape(())
#         return res_var.clamp_min(1e-4)
...
#                 band_importance = self._default_band_importance(spectral_dim, dtype=dtype)
#             else:
#                 band_importance = band_importance / s
...
#         bases = snapshot.get("bases", None)
#         eigvals = snapshot.get("eigvals", None)
#         res_vars = snapshot.get("res_vars", None)
#         variances = snapshot.get("variances", None)
...
#         means = torch.as_tensor(means, device=self.device, dtype=self.means.dtype if self.means.numel() > 0 else torch.float32)
#         bases = torch.as_tensor(bases, device=self.device, dtype=means.dtype)
#         eigvals = torch.as_tensor(eigvals, device=self.device, dtype=means.dtype)
#         res_vars = torch.as_tensor(res_vars, device=self.device, dtype=means.dtype)
...
#         C = means.size(0)
#         self.means = means
#         self.bases = bases[:, :, :self.rank]
#         if self.bases.size(2) < self.rank:
#             pad = torch.zeros(C, self.d_model, self.rank - self.bases.size(2), device=self.device, dtype=means.dtype)
#             self.bases = torch.cat([self.bases, pad], dim=2)

#         self.eigvals = eigvals[:, :self.rank].clamp_min(1e-4)
#         if self.eigvals.size(1) < self.rank:
#             pad = torch.full((C, self.rank - self.eigvals.size(1)), 1e-4, device=self.device, dtype=means.dtype)
#             self.eigvals = torch.cat([self.eigvals, pad], dim=1)

#         self.res_vars = res_vars.flatten().clamp_min(1e-4)
...
#         mean = self._prepare_mean(mean)
#         basis = self._sanitize_basis(basis)
#         eigvals = self._prepare_eigvals(eigvals)
#         res_var = self._prepare_res_var(res_var)
#         spectral_proto, band_importance = self._prepare_spectral_pair(
#             spectral_proto=spectral_proto,
#             band_importance=band_importance,
...
#             self.band_importances = torch.cat([self.band_importances, band_importance.unsqueeze(0)], dim=0)
#         else:
#             # keep row count aligned even when there is no spectral metadata
#             self.spectral_protos = torch.empty((len(self), 0), device=self.device, dtype=mean.dtype)
#             self.band_importances = torch.empty((len(self), 0), device=self.device, dtype=mean.dtype)
...
#         while len(self) < count:
#             dummy_mean = torch.zeros(self.d_model, device=self.device, dtype=dtype)
#             dummy_basis = torch.zeros(self.d_model, self.rank, device=self.device, dtype=dtype)
#             dummy_eigvals = torch.full((self.rank,), 1e-4, device=self.device, dtype=dtype)
#             dummy_res_var = torch.tensor(1e-4, device=self.device, dtype=dtype)
...
#         feat_n = F.normalize(feat, dim=-1, eps=1e-6)
#         means_n = F.normalize(means.to(feat.device, feat.dtype), dim=-1, eps=1e-6)
#         weights = torch.softmax(torch.matmul(feat_n, means_n.t()), dim=-1)
#         return means, weights
